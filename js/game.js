...
export class Game {
    ...
    *nearbyTiles() {
    ...
    get hue() {
        ...
        if (this.level.theme === 'jungle') return 112 + (this.levelIndex % 5) * 9;
    ...
    drawBackground() {
        ...
        // Džungle má místo mřížky koruny stromů, kmeny v mlze a liány
        if (this.level.theme === 'jungle') {
            this.drawJungle();
            return;
        }
    ...
    /**
     * Džungle. Nebe skoro není vidět: nahoře leží vrstvy listí, mezi kterými
     * propadají šikmé pruhy světla, v hloubce stojí kmeny a nad zemí visí mlha.
    ...
    drawJungle() {
        ...
        ctx.fillRect(0, ground, this.c.width, this.c.height - ground);
    }
    ...
    bakeBlock(variant, capped) {
        ...
        const paint = {ice: this.paintIceBlock, desert: this.paintSandBlock,
                       math: this.paintMathBlock, jungle: this.paintJungleBlock,
                      }[this.level.theme] ?? this.paintPlainBlock;
     ...
     */
    paintJungleBlock(ctx, variant, capped) {
        const t = this.tile;
        const s = this.blockSize;
    ...
    // kaktus/sup v pouštním, operátor Δ/∇ v matematickém,
    // masožravá rostlina/had v džungli, jinak klasický
    drawHazard(x, y, up) {
        ...
        } else if (this.level.theme === 'jungle') {
            if (up) this.drawCarnivore(x, y);
            else this.drawSnake(x, y);
        } else {
